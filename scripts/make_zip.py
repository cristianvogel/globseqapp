...
def  main():
  ...
  installer = "\installer\GlobSeqPlugIn Installer.exe"
   
  if demo:
    installer = "\installer\GlobSeqPlugIn Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\GlobSeqPlugIn manual.pdf" 
  ]
  ...
  ZIPNAME = "GlobSeqPlugIn-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "GlobSeqPlugIn-v" + FULLVERSIONSTR + "-win-demo.zip"
